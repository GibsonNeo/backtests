...
def _rolling_sma(x, w): return x.rolling(window=w, min_periods=w).mean()
...
def main():
    ...
    # Run for each exit-day variant
    for i, exits in enumerate(exit_day_variants):
        label = f"exitset_{i}_days{list(exits.values())[0]}"
        ...
        for t in tickers:
            sig_px = data[t]["sig"]
            tr_px = data[t]["tr"]
            pos = hybrid_position(sig_px, tier_windows, entry_days_by_window, exits)
    ...
    # Optional baselines, aligned to the same common index as the last run, or recompute a union
    if include_baseline_buyhold or include_baseline_sma200:
        # Build a broad common index
        common_index = None
        for t in tickers:
            idx = data[t]["tr"].index
        ...
        if include_baseline_buyhold:
            ...
            # no overlay here, this sweep is about exits
        ...
        if include_baseline_sma200:
            ...
            for t in tickers:
                ...
                p200 = sleeve_position(sig_px, window=200, entry_days=0, exit_days=0).reindex(common_index).fillna(0.0)
    ...
    df = df[["variant", "CAGR", "AnnReturn", "AnnVol", "Sharpe", "MaxDD", "UlcerIndex", "TotalReturn"]]
    df.to_csv(os.path.join(outdir, "overall_exit_comparison.csv"), index=False)

    if write_per_year and per_year_rows:
        yr = pd.concat(per_year_rows, ignore_index=True)
        yr.to_csv(os.path.join(outdir, "per_year_exit_comparison.csv"), index=False)

    print(f"Wrote {os.path.join(outdir, 'overall_exit_comparison.csv')}")
    if write_per_year:
        print(f"Wrote {os.path.join(outdir, 'per_year_exit_comparison.csv')}")
