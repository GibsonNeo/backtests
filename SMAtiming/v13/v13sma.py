...
def hybrid20_position(price_signal: pd.Series,
    ...
    Regime A, short SMA at or above long SMA, standard long SMA logic
      entry, price above SMA200 for entry_days consecutive closes
      exit, price at or below SMA200 for exit_days_long consecutive closes

    Regime B, short SMA below long SMA, early reentry with a momentum gate
      entry, price above SMA20 for entry_days consecutive closes, and SMA20 slope up for entry_days consecutive days
      exit, first close at or below SMA20, counted by exit_days_short, then exit next session
    ...
    exit_long_ok  = _consec_true(~aboveL, exit_days_long)   # at or below SMA200
    exit_short_ok = _consec_true(~aboveS, exit_days_short)  # at or below SMA20
...
def main():
    ...
    for (run_start, run_end) in sample_windows:
        ...
        # 200SMA_3in_2out_hybrid20SMA, config driven
