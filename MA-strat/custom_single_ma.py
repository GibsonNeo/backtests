...
Choose one moving average type (SMA, EMA, TEMA)
Allow separate lengths for price filter and slope filter
...
class Params:
    ...
    end_date: Optional[str] = "2025-09-01"
    ticker: Optional[str] = "qqq"
    ma_type: str = "sma"               # sma, ema, tema
    ma_len: int = 200                  # default length if the two below are not set
    ma_len_price: Optional[int] = 200 # if set, used for price above or below tests - None means use ma_len
    ma_len_slope: Optional[int] = 20 # if set, used for slope up or down tests - None means use ma_len
    use_ema_for_tema: bool = True      # placeholder, TEMA uses EMA internally
...
def build_rules(df: pd.DataFrame, p: Params):
    ...
    # choose lengths for each purpose
...
def maybe_plot(df: pd.DataFrame, ctx: pd.DataFrame, equity: pd.DataFrame, p: Params):
    ...
    plt.plot(ctx.index, ctx["MA_price"], label=f"{p.ma_type.upper()} price len")
    plt.plot(ctx.index, ctx["MA_slope"], label=f"{p.ma_type.upper()} slope len")
    ax = plt.gca()
    ax2 = ax.twinx()
...
def main(argv=None):
    import argparse
    ap = argparse.ArgumentParser(description="Single MA backtest, price and slope can use different lengths")
    ...
    ap.add_argument("--ma_len", type=int, default=None, help="fallback length for both price and slope")
    ap.add_argument("--ma_len_price", type=int, default=None, help="length for price above or below tests")
    ap.add_argument("--ma_len_slope", type=int, default=None, help="length for slope up or down tests")
    ...
    if args.ma_len_price is not None:
        p.ma_len_price = args.ma_len_price
    if args.ma_len_slope is not None:
        p.ma_len_slope = args.ma_len_slope
    ...
    tag_core = f"{p.ticker or 'data'}_{p.ma_type.upper()}"
    lp = p.ma_len_price if p.ma_len_price is not None else p.ma_len
    ls = p.ma_len_slope if p.ma_len_slope is not None else p.ma_len
    tag = f"{tag_core}_P{lp}_S{ls}_{p.entry_rule}2{p.exit_rule}_{p.timeframe}".replace(" ", "")
