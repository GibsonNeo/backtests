...
#!/usr/bin/env python3
"""
MA exit variants report
Window isolated backtests, start flat each window, force close on last bar.
All strategies use entry based on SMA 200 slope rising.
Exits vary by strategy:
  S2, price below SMA 200
  S1, SMA 200 slope down
  S3, price below SMA 200 OR SMA 200 slope down, whichever happens first

Everything uses Close for signals and buy and hold.
...
def load_data(ticker: str, start: Optional[str], end: Optional[str]) -> pd.DataFrame:
    if yf is None:
        raise RuntimeError("yfinance is not available. Install it or provide CSVs.")
...
class StratParams:
    ...
    ma_len_slope: Optional[int] = 200   # entry slope uses 200
    entry_rule: str = "slope_up"        # fixed to slope_up on 200
    exit_rule: str = "price_below"      # price_below, slope_down, price_below_or_slope_down
...
def build_workbook(out_path: str, summary_df: pd.DataFrame, windows_df: pd.DataFrame, per_ticker_df: pd.DataFrame, portfolio_df: pd.DataFrame):
...
def compute_all(
    ...
    windows_map = pd.DataFrame(windows_df)[["window_id","regime"]]
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="MA exit variants report")
    ap.add_argument("--config", type=str, required=True, help="Path to YAML config, example exit_variants.yml")
    ...
    out_path = cfg.get("output", {}).get("workbook_path", f"exit_variants_{universe_label}_{ts}.xlsx")
