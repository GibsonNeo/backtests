...
#!/usr/bin/env python3
"""
MA exit confirmations report
All strategies use the same entry base, SMA 200 slope up.
Exit variants add confirmation bars on entry, exit, or both.

S1  entry confirm 1, exit confirm 2, exit rule is first of price below SMA 200 or SMA 200 slope down
S2  entry confirm 2, exit confirm 1, exit rule is first of price below SMA 200 or SMA 200 slope down
S3  entry confirm 1, exit confirm 1, exit rule is first of price below SMA 200 or SMA 200 slope down
S4  entry confirm 2, exit confirm 2, exit rule is first of price below SMA 200 or SMA 200 slope down

Window isolated, start flat per window, force close at the last bar.
Signals and buy and hold use Close for apples to apples.
Portfolio is equal weight across active names each day.
"""
...
class StratParams:
    ...
    entry_rule: str = "slope_up"         # fixed to slope_up on 200
    exit_rule: str = "price_below_or_slope_down"
...
def build_rules(price: pd.Series, p: StratParams):
    ...
    if p.exit_rule == "slope_down":
        ...
        exit_raw = below
    elif p.exit_rule == "price_below_or_slope_down":
        exit_raw = below | slope_down
    else:
        raise ValueError("exit_rule must be price_below, slope_down, or price_below_or_slope_down")
...
def build_workbook(
    ...
    summary_vs_s3_df: pd.DataFrame,
):
    with pd.ExcelWriter(out_path, engine="xlsxwriter") as writer:
        summary_df.to_excel(writer, sheet_name="Summary", index=False)
        _auto_width_and_formats(writer, "Summary", summary_df)

        # add SummaryPlus with comparisons versus S3 baseline
        summary_vs_s3_df.to_excel(writer, sheet_name="SummaryPlus", index=False)
        _auto_width_and_formats(writer, "SummaryPlus", summary_vs_s3_df)
...
def compute_all(
    ...
    # Head to head win rates and average differences, all pairs
    ...
    # Regime split win rates, for each pair
    ...
    # SummaryPlus, add columns that compare each strategy to S3 baseline
    base = "S3_Entry200SlopeUp_ExitPriceBelowORslopeDown200"
    ...
    for strat in names:
        ...
        plus_rows.append({
            "strategy": strat,
            "win_sharpe_vs_S3_pct": win_sharpe,
            "win_cagr_vs_S3_pct": win_cagr,
            "better_mdd_vs_S3_pct": better_mdd,
            "avg_excess_sharpe_vs_S3": d_sh,
            "avg_excess_cagr_vs_S3": d_cg,
            "avg_mdd_diff_vs_S3": d_mdd,
        })
    summary_vs_s3_df = summary_df.merge(pd.DataFrame(plus_rows), on="strategy", how="left")

    return summary_df, windows_df, per_ticker_df, portfolio_df, head_to_head_df, regime_winrates_df, summary_vs_s3_df


def main(argv=None):
    ap = argparse.ArgumentParser(description="MA exit confirmations report")
    ap.add_argument("--config", type=str, required=True, help="Path to YAML config, example exit_confirmations.yml")
    ...
    for s in strategies_cfg:
        strategies.append(StratParams(
            ...
            entry_rule="slope_up",
            exit_rule=str(s.get("exit_rule", "price_below_or_slope_down")),
    ...
    summary_df, windows_df, per_ticker_df, portfolio_df, head_to_head_df, regime_winrates_df, summary_vs_s3_df = compute_all(
    ...
    out_path = cfg.get("output", {}).get("workbook_path", f"exit_confirms_{universe_label}_{ts}.xlsx")
    build_workbook(out_path, summary_df, windows_df, per_ticker_df, portfolio_df,
                   head_to_head_df, regime_winrates_df, summary_vs_s3_df)
