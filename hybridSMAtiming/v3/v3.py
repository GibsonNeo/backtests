...
# ---------------- strategy with cooldown and split entry streaks ----------------
def hybrid_position_with_cooldown(
...
def main():
    ...
    # new split entry defaults, keep backward compatible with entry_days
    ...
    grid = [
        (L, S, C, eL, eS, xL, xS)
        for L, S, C, eL, eS, xL, xS in product(
            long_list, short_list, cd_list, entry_long_list, entry_short_list, exit_long_list, exit_short_list
        )
        if not excluded(L, S, C, eL, eS, xL, xS)
    ]
    ...
    for (run_start, run_end) in sample_windows:
        ...
        # run each variant
        for L, S, C, eL, eS, xL, xS in grid:
            name = f"hybridL{L}_S{S}_cd{C}_eL{eL}_eS{eS}_xL{xL}_xS{xS}"
