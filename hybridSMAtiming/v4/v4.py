...
def fetch_series(ticker, start, end_inclusive):
    ...
    # keep original split adjustment approach from your version
...
def hybrid_position_no_cooldown(
    ...
    Regime A (short SMA at or above long SMA)
      entry, price above long SMA for entry_days_long consecutive closes
      exit, price at or below long SMA for exit_days_long consecutive closes

    Regime B (short SMA below long SMA)
      entry, price above short SMA and short SMA rising, both true for entry_days_short consecutive days
      exit, price at or below short SMA for exit_days_short consecutive closes

    No cooldown, exits can be followed by immediate new entries if conditions are met
    Fills occur next session via the final shift
    """
...
def main():
    ...
    # split entry defaults, keep backward compatibility with entry_days
    base_entry_long = int(defaults.get("entry_days_long", defaults.get("entry_days", 3)))
    base_entry_short = int(defaults.get("entry_days_short", defaults.get("entry_days", 3)))

    base_exit_long = int(defaults.get("exit_days_long", 2))
    base_exit_short = int(defaults.get("exit_days_short", 1))

    long_list = [int(x) for x in grids.get("long_windows", [base_long])]
    short_list = [int(x) for x in grids.get("short_windows", [base_short])]

    # cooldown grid removed, completely ignored
    entry_long_list = [int(x) for x in grids.get("entry_days_long", [base_entry_long])]
    entry_short_list = [int(x) for x in grids.get("entry_days_short", [base_entry_short])]
    exit_long_list = [int(x) for x in grids.get("exit_days_long", [base_exit_long])]
    exit_short_list = [int(x) for x in grids.get("exit_days_short", [base_exit_short])]

    def excluded(L, S, eL, eS, xL, xS):
        # cooldown keys in exclusions are ignored
        for e in exclusions:
            ok = True
            if "long" in e: ok = ok and int(e["long"]) == L
            if "short" in e: ok = ok and int(e["short"]) == S
            if "eL" in e: ok = ok and int(e["eL"]) == eL
            if "eS" in e: ok = ok and int(e["eS"]) == eS
            if "xL" in e: ok = ok and int(e["xL"]) == xL
            if "xS" in e: ok = ok and int(e["xS"]) == xS
            if ok:
                return True
        return False

    # fetch data, extend back for warmup across the grid
    ...
    grid = [
        (L, S, eL, eS, xL, xS)
        for L, S, eL, eS, xL, xS in product(
            long_list, short_list, entry_long_list, entry_short_list, exit_long_list, exit_short_list
        )
        if not excluded(L, S, eL, eS, xL, xS)
    ]

    for (run_start, run_end) in sample_windows:
        print(f"\nRunning window {run_start} to {run_end}")

        # baseline buy and hold once per window
        ...
        # run each variant
        for L, S, eL, eS, xL, xS in grid:
            name = f"hybridL{L}_S{S}_eL{eL}_eS{eS}_xL{xL}_xS{xS}"
            ...
            for t in tickers:
                ...
                pos = hybrid_position_no_cooldown(
                    sig_px,
                    w_long=L,
                    w_short=S,
                    entry_days_long=eL,
